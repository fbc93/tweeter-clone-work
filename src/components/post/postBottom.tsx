...
const PostBottom = ({ likes, comments, views, shares, onClickLike, isLiked, reply, onClickCommentLike, commentId }: PostBottomProps) => {
  return (
    <>
      {/* 댓글 */}
      {comments >= 0 && (
      ...
      {/* 좋아요 */}
      {likes >= 0 ? (
        <div comment-id={commentId} onClick={onClickLike ? onClickLike : onClickCommentLike} className={`w-1/3 flex items-center cursor-pointer ${reply ? "justify-end" : "justify-center"}`}>
          <div className="mr-2 text-sm">
      ...
      {/* 대댓글 */}
      {reply && (
        <div className="w-1/3 flex justify-end items-center cursor-pointer">
          <div className="text-sm">
            <svg className="w-5 h-5 shadow-lg text-white/50" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
